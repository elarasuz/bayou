use bayou::commands::Cli;
use bayou::config::Settings;
...
// use config::Config;
use file_rotate::{compression::Compression, suffix::AppendTimestamp, ContentLimit, FileRotate, TimeFrequency};
...
async fn mqtt_stream_topic(cfg: &Settings) {
  let mut mqttoptions = MqttOptions::new("bayou", &cfg.mqtt.host, cfg.mqtt.port);
  ...
  let mut log = FileRotate::new(
    &cfg.logs.path,
    // AppendCount::new(10),
    AppendTimestamp::with_format("%Y%m%dT%H", FileLimit::MaxFiles(cfg.logs.files), DateFrom::Now),
  ...
  client.subscribe(&cfg.mqtt.topic, QoS::AtMostOnce).await.unwrap();
  info!("[{}] Subscribed", cfg.mqtt.topic);
  ...
  loop {
    ...
    match res {
      ...
      Err(ConnectionError::Io(error)) => {
        ...
        println!("Connection IO error: {error:?}");
        exit(1);
      }
